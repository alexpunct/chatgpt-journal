<script lang="ts">
	import { menuNavLinks } from '../Navigation/links';
	...
	const cList =
		'p-4 space-y-4 rounded-container-token max-h-[60vh] md:max-h-[75vh] overflow-y-auto';
	const cCardLink =
		'card bg-surface-200-700-token p-2 grid grid-cols-[auto_1fr_auto_auto] gap-4 items-center';
	const cCardLinkFocus =
		'hover:bg-primary-500 hover:text-on-primary-token focus:bg-primary-500 focus:text-on-primary-token';
	const cHashtag =
		'border-2 border-surface-300-600-token rounded w-7 aspect-square flex justify-center items-center text-xs';
